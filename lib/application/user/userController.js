...
module.exports = {
  ...
  async addUser (req, res) {
    ...
    const pseudoAlreadyRegistered = await User.findOne({ pseudo: user.pseudo })
    const emailAlreadyRegistered = await User.findOne({ email: user.email })

    if (emailAlreadyRegistered) return res.status(409).send({ message: 'email already used' })
    if (pseudoAlreadyRegistered) return res.status(409).send({ message: 'pseudo already used' })
