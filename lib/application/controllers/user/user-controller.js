...
module.exports = {
  ...
  async addUser (req, res) {
    ...
    if (!user.isPasswordStrongEnough()) return res.status(409).send({ message: 'invalidPassword' })

    if (!user.isNewPseudoValid()) return res.status(409).send({ message: 'invalidPseudo' })
