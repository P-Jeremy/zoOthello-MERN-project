...
describe('Acceptance | Api | gameRouter', () => {
  ...
  describe('#resetGame', () => {
    it('should reset a game', (done) => {
      ...
      // when
      chai.request(app)
        .put(`/api/game/resetGame/${id}`)
        .send(data)
        .end((_err, res) => {
          // then
          res.body.should.be.a('object')
          res.body.should.have.property('_id', id)
          res.body.blackPassCount.should.equal(0)
          res.body.whitePassCount.should.equal(0)
          expect(res).to.have.status(200)
          done()
        })
    })
